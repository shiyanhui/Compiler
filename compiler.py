...
class Parser( object ):
	...
	def _function_statement( self, father=None ):
		...
		while flag and self.index < len( self.tokens ):
			...
			if self.tokens[ self.index ].value in keywords[ 0 ]:
				...
				func_statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'FIELD_TYPE', { 'type': self.tokens[ self.index ].value } ) )
				self.index += 1
			#如果是函数名
				...
				#extra_info
				func_statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'IDENTIFIER', { 'type':'FUNCTION_NAME' } ) )
				self.index += 1
			#如果是参数序列
				...
				while self.tokens[ self.index ].type != 'RL_BRACKET':
					if self.tokens[ self.index ].value in keywords[ 0 ]:
						...
						#extra_info
						func_statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'FIELD_TYPE', { 'type':self.tokens[ self.index ].value } ), param )
						if self.tokens[ self.index + 1 ].type == 'IDENTIFIER':
							#extra_info
							func_statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index + 1 ].value, 'IDENTIFIER', { 'type':'VARIABLE', 'variable_type':self.tokens[ self.index ].value } ), param )
						else:
							print '函数定义参数错误！'
							exit()
						self.index += 1
	...
	def _statement( self, father=None ):
		...
		while self.tokens[ self.index ].type != 'SEMICOLON':
			...
			if self.tokens[ self.index ].value in keywords[ 0 ]:
				...
				#extra_info
				statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'FIELD_TYPE', { 'type':self.tokens[ self.index ].value } ) )
			#变量名
			elif self.tokens[ self.index ].type == 'IDENTIFIER':
				#extra_info
				statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'IDENTIFIER', { 'type':'VARIABLE', 'variable_type':tmp_variable_type } ), statement_tree.root )
			#数组大小
			elif self.tokens[ self.index ].type == 'DIGIT_CONSTANT':
				statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'DIGIT_CONSTANT' ), statement_tree.root )
				statement_tree.current.left.set_extra_info( { 'type':'LIST', 'list_type':tmp_variable_type } )
			#数组元素
			elif self.tokens[ self.index ].type == 'LB_BRACKET':
				self.index += 1
				...
				while self.tokens[ self.index ].type != 'RB_BRACKET':
					if self.tokens[ self.index ].type == 'DIGIT_CONSTANT':
						statement_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'DIGIT_CONSTANT' ), constant_list )
					self.index += 1
			#多个变量声明
				...
				while self.tokens[ self.index ].type != 'SEMICOLON':
					if self.tokens[ self.index ].type == 'IDENTIFIER':
						...
						#extra_info
						#类型
						tree.add_child_node( SyntaxTreeNode( tmp_variable_type, 'FIELD_TYPE', { 'type': tmp_variable_type } ) ) 
						#变量名
						tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'IDENTIFIER', { 'type':'VARIABLE', 'variable_type':tmp_variable_type } ), tree.root )
					self.index += 1
				break
			self.index += 1
		self.index +=1
	...
	def _assignment( self, father=None ):
		...
		while self.tokens[ self.index ].type != 'SEMICOLON':
			#被赋值的变量
			if self.tokens[ self.index ].type == 'IDENTIFIER':
				assign_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'IDENTIFIER' ) )	
				self.index += 1	
	...
	def _expression( self, father=None, index=None ):
		...
		while self.tokens[ self.index ].type != 'SEMICOLON':
			...
			if self.tokens[ self.index ].type == 'DIGIT_CONSTANT':
				...
				tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, '_Constant' ) )
			...
			#如果是变量或者数组的某元素
				...
				if self.tokens[ self.index + 1 ].value in operators or self.tokens[ self.index + 1 ].type == 'SEMICOLON':
					...
					tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, '_Variable' ) )
					reverse_polish_expression.append( tree )
				#数组的某一个元素ID[i]
				elif self.tokens[ self.index + 1 ].type == 'LM_BRACKET':
					tree = SyntaxTree()
					tree.current = tree.root = SyntaxTreeNode( 'Expression', 'ArrayItem' )
					#数组的名字
					tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, '_ArrayName' ) )
					self.index += 2
					if self.tokens[ self.index ].type != 'DIGIT_CONSTANT' and self.tokens[ self.index ].type != 'IDENTIFIER':
						...
						#数组下标
						tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, '_ArrayIndex' ), tree.root )
						reverse_polish_expression.append( tree )
			#如果是运算符
				...
				tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, '_Operator' ) )	
				#如果是左括号，直接压栈
				if self.tokens[ self.index ].type == 'LL_BRACKET':
		...
		for item in reverse_polish_expression:
			if item.root.type != 'Operator':
				...
				if item.current.value in child_operators[ 0 ] :
					a = operand_stack.pop()
					new_tree = SyntaxTree()
					new_tree.current = new_tree.root = SyntaxTreeNode( 'Expression', 'SingleOperand' )
					#添加操作符
	...
	def _function_call( self, father=None ):
		...
		while self.tokens[ self.index ].type != 'SEMICOLON':
			#函数名
			if self.tokens[ self.index ].type == 'IDENTIFIER':
				func_call_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'FUNCTION_NAME' ) )
			#左小括号
			elif self.tokens[ self.index ].type == 'LL_BRACKET':
				self.index += 1
				...
				while self.tokens[ self.index ].type != 'RL_BRACKET':
					if self.tokens[ self.index ].type == 'IDENTIFIER' or self.tokens[ self.index ].type == 'DIGIT_CONSTANT' or self.tokens[ self.index ].type == 'STRING_CONSTANT':
						...
						func_call_tree.add_child_node( SyntaxTreeNode( self.tokens[ self.index ].value, 'ADDRESS' ), params_list )
					self.index += 1
			else:
...
class Assembler( object ):
	def __init__( self ):
		...
		self.symbol_table = { }
		#语法类型
		self.sentence_type = [ 'Sentence', 'Include', 'FunctionStatement', 'Statement', 'FunctionCall', 'Assignment', 'Control', 'Expression', 'Return' ]
		#表达式中的符号栈
		self.operator_stack = [ ]
		#表达式中的操作符栈
		self.operand_stack = [ ]
	...
	def _statement( self, node=None ):
		...
		while current_node:
			...
			#变量名
				...
				line = '.lcomm ' + variable_name + ', ' + self._sizeof( variable_field_type )
			#数组元素
				...
				line += ', '.join( array )
			current_node = current_node.right
		self.ass_file_handler.insert( line, 'BSS' if variable_type == 'VARIABLE' else 'DATA' )
		#将该变量存入符号表
		self.symbol_table[ variable_name ] = { 'type':variable_type, 'field_type':variable_field_type }
				...
				while tmp_node:
					...
					if tmp_node.type == 'DIGIT_CONSTANT' or tmp_node.type == 'STRING_CONSTANT':
						...
						if tmp_node.type == 'STRING_CONSTANT':
							...
							exit()
						self.symbol_table[ label ] = { 'type':'STRING_CONSTANT', 'value':tmp_node.value }
		...
		if current_node.type == 'IDENTIFIER' and current_node.right.value == 'Expression':
			...
			if field_type == 'int':
				...
				if expres[ 'type' ] == 'CONSTANT':
					line = 'movl $' + expres[ 'value' ] + ', ' + current_node.value
					self.ass_file_handler.insert( line, 'TEXT' )
	...
	def _traverse_expression( self, node=None ):
		...
		if node.type == '_Variable':
			self.operand_stack.append( { 'type':'VARIABLE', 'operand':node.value } )
		elif node.type == '_Constant':
			self.operand_stack.append( { 'type':'CONSTANT', 'operand':node.value } )
		elif node.type == '_Operator':
			self.operator_stack.append( node.value )
		elif node.type == '_ArrayName':
			self.operand_stack.append( { 'type':'ARRAY_ITEM', 'operand':[ node.value, node.right.value ] } )
			return
		current_node = node.first_son
		while current_node:
	...
	def _expression( self, node=None ):
		...
		while self.operator_stack:
			...
			if operator in double_operators:
				...
				if operator == '+':
					if contain_float:
						line = 'flds ' if self._is_float( operand_a ) else 'filds '
						line += operand_a[ 'operand' ]
						self.ass_file_handler.insert( line, 'TEXT' )
						line = 'fadd ' if self._is_float( operand_b ) else 'fiadd '
						...
						self.ass_file_handler.insert( line, 'TEXT' )
						#计算结果压栈
						self.operand_stack.append( { 'type':'VARIABLE', 'operand':'bss_tmp' } )
						#记录到符号表中
						self.symbol_table[ 'bss_tmp' ] = { 'type':'IDENTIFIER', 'field_type':'float' }
					else:
						#第一个操作数
						if operand_a[ 'type' ] == 'ARRAY_ITEM':
							line = 'movl ' + operand_a[ 'operand' ][ 1 ] + r', %edi'
							self.ass_file_handler.insert( line, 'TEXT' )
							line = 'movl ' + operand_a[ 'operand' ][ 0 ] + r'(, %edi, 4), %eax'
							self.ass_file_handler.insert( line, 'TEXT' )
						...
						self.operand_stack.append( { 'type':'VARIABLE', 'operand':'bss_tmp' } )
						#记录到符号表中
						self.symbol_table[ 'bss_tmp' ] = { 'type':'IDENTIFIER', 'field_type':'int' }
					...
					if contain_float:
						...
						if self._is_float( operand_a ):
							...
							if operand_a[ 'type' ] == 'CONSTANT':
								line = 'movl $' + operand_a[ 'operand' ] + ', bss_tmp'
								self.ass_file_handler.insert( line, 'TEXT' )
							else:
								pass
						#操作数b
						if self._is_float( operand_b ):
							if operand_b[ 'type' ] == 'VARIABLE': 
								line = 'flds ' if self._is_float( operand_b ) else 'filds '
								line += operand_b[ 'operand' ]
								self.ass_file_handler.insert( line, 'TEXT' )
							...
							if operand_b[ 'type' ] == 'CONSTANT':
								line = 'movl $' + operand_b[ 'operand' ] + ', bss_tmp'
								self.ass_file_handler.insert( line, 'TEXT' )
								line = 'fisub bss_tmp'
								self.ass_file_handler.insert( line, 'TEXT' )
							else:
				...
				#尚未考虑浮点数，只考虑整数乘法
					...
					if operand_b[ 'type' ] == 'ARRAY_ITEM':
						...
						self.ass_file_handler.insert( line, 'TEXT' )
						#相乘
						line = 'mull ' + operand_b[ 'operand' ][ 0 ] + '(, %edi, 4)'
						self.ass_file_handler.insert( line, 'TEXT' )
					else:
						print 'other MUL not supported yet!'
						exit()
					...
					if contain_float:
						...
						line = 'fdiv ' if self._is_float( operand_b ) else 'fidiv '
						line += operand_b[ 'operand' ]
						self.ass_file_handler.insert( line, 'TEXT' )
					...
					if contain_float:
						...
						if self._is_float( operand_b ):
							...
							if operand_b[ 'type' ] == 'CONSTANT':
								...
								line = 'fcom bss_tmp'
								self.ass_file_handler.insert( line, 'TEXT' )
								line = operator_map[ '>=' ] + ' ' + self.labels_ifelse[ 'label_else' ]
								self.ass_file_handler.insert( line, 'TEXT' )
							else:
								pass
					else:
					...
					if contain_float:
						...
						line = 'movl $' if operand_a[ 'type' ] == 'CONSTANT' else 'movl '
						line += operand_a[ 'operand' ] + ', %edi'
						self.ass_file_handler.insert( line, 'TEXT' )

						line = 'movl $' if operand_b[ 'type' ] == 'CONSTANT' else 'movl '
						...
						line = operator_map[ '<' ] + ' ' + 'label_' + str( self.label_cnt )
						self.ass_file_handler.insert( line, 'TEXT' )
				...
				exit()
		result = { 'type':self.operand_stack[0]['type'], 'value':self.operand_stack[0]['operand'] } if self.operand_stack else { 'type':'', 'value':''}
		return result
